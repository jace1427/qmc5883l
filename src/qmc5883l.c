...
int _qmc5883l_write_one_byte(const uint8_t reg, const uint8_t val) {
  ...
  return ret > 0 ? PICO_OK : PICO_ERROR_GENERIC;
}
...
int qmc5883l_read_data(QMC5883L *qmc) {
  ...
  for (int i = 0; i < 100; i++) {
    if (!ret) {
      ...
      _qmc5883l_read_two_bytes(REG_XOUT_LSB, REG_XOUT_MSB, &qmc->data.x);
      _qmc5883l_read_two_bytes(REG_YOUT_LSB, REG_YOUT_MSB, &qmc->data.y);
      _qmc5883l_read_two_bytes(REG_ZOUT_LSB, REG_ZOUT_MSB, &qmc->data.z);
      if (qmc->calibrated.status) {
 ...
 */
int _qmc5883L_set_config(QMC5883L *qmc, const int odr, const int rng,
...
/*! Resets the QMC5883L, and sets it to continous data read mode.
 *  /ingroup qmc5883l
 *
 * /param qmc A pointer to the qmc struct.
 *
...
int _qmc5883l_init_internal(QMC5883L *qmc) {
  ...
  uint8_t chipID;
  if (_qmc5883l_read_one_byte(REG_CHIP_ID, &chipID)) {
    printf("Error getting ChipID!\n");
  } else if (chipID != CHIP_ID) {
    printf("Wrong ChipID: 0x%x!\n", chipID);
  }

  int ret = PICO_OK;
  // soft reset - restore defulat value of all registers
...
int qmc5883l_init(QMC5883L *qmc, const int odr, const int rng, const int osr,
  ...
  if (!_qmc5883L_set_config(qmc, odr, rng, osr, degrees, minutes)) {
    return PICO_ERROR_INVALID_ARG;
  }
